...
layouts["modern"] = function()
	...
	if
		...
		and ((user_opts.window_top_bar == "yes" or not (state.border and state.title_bar)) or state.fullscreen)
		and top_titlebar
	then
	...
	geo = {
		x = 25,
		y = refY - (chapter_index and 102 or 90),
		an = 1,
	...
	local narrow_vid_tc_y = osc_param.playresx
		< (
			930
			- outeroffset
	...
	lo.geometry = {
		x = (narrow_vid_tc_y and refX or time_codes_x),
		y = refY - (narrow_vid_tc_y and 57 or 35),
		an = (narrow_vid_tc_y and 5 or 4),
		w = 90 + tc_w_offset,
		h = 15,
	}
	...
	if user_opts.cache_info then
		...
		lo.geometry = {
			x = osc_geo.w - (cache_speed and 345 or 340) + cache_x_offset,
			y = refY - (cache_speed and 41 or 35),
			an = 6,
			w = 35,
			h = 24,
		}
...
local function osc_init()
	...
	ne.content = function()
		if user_opts.chapter_fmt ~= "no" and chapter_index >= 0 then
			...
			local chapter_title = (chapters[chapter_index + 1] and chapters[chapter_index + 1].title ~= "")
				...
				or locale.chapter .. ": " .. chapter_index + 1 .. "/" .. #chapters
			chapter_title = mp.command_native({ "escape-ass", chapter_title })
			chapter_title = (thumbfast.disabled and not user_opts.show_title)
					and (state.forced_title ~= nil and state.forced_title)
				or chapter_title
	...
	ne.eventresponder["mbtn_left_up"] = function()
		mp.commandv("osd-msg", "set", "speed", math.min(100, mp.get_property_number("speed") + 1))
	end
	ne.eventresponder["mbtn_right_up"] = function()
		mp.commandv("osd-msg", "set", "speed", 1)
	end
	ne.eventresponder["wheel_up_press"] = function()
		mp.commandv("osd-msg", "set", "speed", math.min(100, mp.get_property_number("speed") + 0.25))
	end
	ne.eventresponder["wheel_down_press"] = function()
		mp.commandv("osd-msg", "set", "speed", math.max(0.25, mp.get_property_number("speed") - 0.25))
	end
	...
	-- cache info
	local cache_state_ranges = state.cache_state
		and state.cache_state["seekable-ranges"]
		and #state.cache_state["seekable-ranges"] > 0
	...
	ne.content = function()
		...
		if not cache_state_ranges then
			return ""
		end
		...
		return state.buffering and locale.buffering .. ": " .. mp.get_property("cache-buffering-state") .. "%"
			or cache_time
	end
	ne.tooltip_style = osc_styles.tooltip
...
local function render()
	...
	-- submit
	set_osd(osc_param.playresy * osc_param.display_aspect, osc_param.playresy, ass.text, -1)
end
